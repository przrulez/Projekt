...
if(@$_GET['str'] == "logout") {
	...
	header("Location: p04/..");
} else if(@$_POST['str'] == "login") {
	if(@$_POST['login']=="admin" && @$_POST['password']=="admin") {
		$_SESSION['logged'] = 1;
		//echo "<script>alert('".$_SESSION['logged']."'); window.location.href='index.php';</script>";
		//header("Location: http://www.foto-agent.pl/p04/");
	} else {
				...
				<body>
				  <div id='container'>
					...
					<div id='content' style='text-align: center;'>
						$message
						<form action='' method='POST' style='width: 200px; margin: 20px auto;'>
							<label for='login'>Login</label>
							<input type='text' name='login' />
							<label for='password'>Password</label>
							<input type='password' name='password' />
							<input type='submit' value='Login' />
							<input type='hidden' name='str' value='login' />
						  </form>
					</div>
				  </div>
				</body>
